#!/bin/bash
openssl req -x509 -nodes -out /etc/nginx/ssl/inception.crt -keyout /etc/nginx/ssl/inception.key -subj "/C=MO/ST=khouribga/L=khouribga/O=inception/CN=souchen.42.fr"
